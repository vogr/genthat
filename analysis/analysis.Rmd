...
# TODO: load count-onexit--sequence
# TODO: load onexit-sequence
```
...
n_traces %>% 
  ...
  mutate(trace_generated=generated/n_complete, generated_passed=passed/generated, trace_passed=passed/n_complete) %>%
  gather(key="key", value="value", trace_generated, generated_passed, trace_passed) %>%
  ggplot(aes(x=key, y=value)) +
  geom_boxplot() +
...
n_traces %>% 
  ...
  left_join(n_generated_tests, by="package") %>%
  left_join(n_run_tests, by="package") %>%
  mutate(generated=generated/n_complete, ran=passed/n_complete) %>%
