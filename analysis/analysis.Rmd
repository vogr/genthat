...
package_stat <- function(path) {
    (2*file.exists(file.path(path, "trace-on.exit--set", "output", "all", "genthat-traces.csv")))^1 +
    (2*file.exists(file.path(path, "trace-on.exit--sequence", "output", "all", "genthat-traces.csv")))^2 +
    (2*file.exists(file.path(path, "trace-onexit--set", "output", "all", "genthat-traces.csv")))^3 +
    (2*file.exists(file.path(path, "trace-onexit--sequence", "output", "all", "genthat-traces.csv")))^4 +
    (2*file.exists(file.path(path, "trace-count-entry--sequence", "output", "all", "genthat-traces.csv")))^5 +
    (2*file.exists(file.path(path, "trace-count-exit--sequence", "output", "all", "genthat-traces.csv")))^6 +
    (2*file.exists(file.path(path, "generate", "output", "genthat-generate.csv")))^7 +
    (2*file.exists(file.path(path, "run", "output", "genthat-run-tests.csv")))^8 +
    (2*file.exists(file.path(path, "run-package", "output", "none", "all", "genthat-traces.csv")))^9
}
...
all_packages <- 
  ...
  mutate(package=basename(path), stat=package_stat(path))

ready_stat <- sum(2^(1:8))
packages <- filter(
  all_packages, 
  bitwAnd(stat, ready_stat) == ready_stat
)
...
**TODO: what happended to the packages that did not make it**
...
n_missing_calls  %>%
  ...
  labs(
    title="Missing calls",
    subtitle="How many calls did we missed?",
    x="Package",
...
n_generated_tests %>%
  ...
  mutate(ratio=ifelse(n_complete > 0, generated / n_complete, 0)) %>%
  ggplot(aes(
  ...
  geom_col() +
  geom_text(aes(label=sprintf("%d", generated), color=ifelse(generated > 0, "white", "black")), hjust=1.5, size=2) +
  geom_text(hjust=-.2, size=2) +
  scale_y_continuous(labels=scales::percent_format()) +
  scale_color_identity() +
  coord_flip() +
  labs(
    title="Percentage of generated tests",
    x="Package",
    y="Generated tests [%]"
  )
```
...
```{r}
run_tests %>% filter((nb - warning) > 1) %>% DT::datatable()
```
