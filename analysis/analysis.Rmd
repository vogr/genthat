...
```{r find packages}
all_package_paths <- list.dirs(RUN_DIR, recursive=FALSE)
all_packages <- basename(all_package_paths)

package_paths <- all_package_paths %>% keep(is_package_ready)
packages <- basename(package_paths)

result <- data_frame(package=packages, path=package_paths)
